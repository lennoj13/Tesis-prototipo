...
/**
 * Estudiante Notificaciones — Lista real de notificaciones.
...
import { FiBriefcase, FiCheckCircle, FiTarget, FiMessageCircle, FiCheck, FiTrash2, FiBell } from 'react-icons/fi';

const iconMap = {
  vacancy: { icon: FiBriefcase, bg: 'bg-primary-50', color: 'text-primary-600' },
  application: { icon: FiCheckCircle, bg: 'bg-success-light', color: 'text-green-600' },
  matching: { icon: FiTarget, bg: 'bg-warning-light', color: 'text-amber-600' },
  message: { icon: FiMessageCircle, bg: 'bg-info-light', color: 'text-blue-600' },
...
export default function EstudianteNotificaciones() {
  ...
  if (loading) {
    return <div className="animate-fade-in p-12 text-center text-slate-400">Cargando notificaciones...</div>;
  }
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-600 m-0 leading-relaxed">{notif.message}</p>
