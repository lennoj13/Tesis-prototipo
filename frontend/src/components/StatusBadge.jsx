 ...
 * Demuestra: Componente de presentación con variantes por props.
 */
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${style}`}>
      <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
        status === 'activo' || status === 'aprobado' ? 'bg-green-500' :
        status === 'pendiente' ? 'bg-amber-500' :
        status === 'rechazado' ? 'bg-red-500' :
        status === 'abierta' || status === 'nuevo' ? 'bg-blue-500' :
        'bg-slate-400'
      }`} />
      {displayLabel}
    </span>
