/**
 * Vista listado Vacantes
 * Componente reutilizable para admin y gestor
 */
...
export default function VacantesListView({
  title,
  ...
  const sortedAndFilteredVacantes = useMemo(() => {
    let result = [...vacantes];

    if (showFacultyFilter && facultadFilter) {
    ...
    return result;
  }, [vacantes, facultadFilter, modalidadFilter, estadoFilter, sortBy, showFacultyFilter]);
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title={title}
        subtitle={loading ? 'Cargando...' : `${vacantes.length} vacantes · ${vacantes.filter((v) => v.activo).length} abiertas`}
      />
