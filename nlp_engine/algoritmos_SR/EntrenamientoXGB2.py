...
# El modelo Transformer de contexto completo se preserva limpio para la dirección semántica
...
def construir_features_semanticas(df_interacciones):
    ...
    for i in range(len(df_interacciones)):
        ...
        # Construcción del vector final para esta fila (Dimensión exacta: 371 features)
...
def entrenar_modelo_xgboost(X, y):
    ...
    print("\n=============================================")
    print("      MÉTRICAS DE EVALUACIÓN XGBOOST V2      ")
    print("=============================================")
    print(f"R² Score (Precisión General) : {r2:.4f}")
    print(f"RMSE (Margen de Error)       : {rmse:.4f}%")
    print("=============================================")
    
    # Consolidado a la carpeta de la versión 2 (xgboost2)
    os.makedirs("../modelos_entrenados/xgboost2", exist_ok=True)
    joblib.dump(model, "../modelos_entrenados/xgboost2/pivipp_xgboost.pkl")
    joblib.dump(scaler, "../modelos_entrenados/xgboost2/pivipp_xgboost_scaler.pkl")
    print("¡Modelo XGBoost V2 guardado exitosamente!")
