...
# ==========================================
# 4. EXTRACCIÓN DE FEATURES SEMÁNTICAS (EMBEDDINGS MULTI-MÉTRICA)
# ==========================================
def construir_features_semanticas(df_interacciones):
    print("\n[INFO] Procesando embeddings y calculando vectores de características ampliados...")
    ...
    print("-> Construyendo vectores combinados para el clasificador (389 Características)...")
    for i in range(len(df_interacciones)):
        emb_v = embs_vacantes[i]
        emb_e = embs_estudiantes[i]
        
        # 4.1. Similitud coseno pura de BERT (Métrica angular)
        ...
        # 4.4. Métricas directas de Skill-Overlap (Matemática de conjuntos)
        ...
        # Unificar el nuevo vector estructurado (1 + 384 + 2 + 2 = 389 columnas en X)
        features_fila = np.concatenate([
            [sim_coseno], 
            diff_vectorial, 
...
def entrenar_modelo_pivipp(X, y):
    ...
    print("Escalando vectores numéricos ampliados...")
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    
    print("Entrenando regresor RandomForest (Alineando pesos semánticos y distancias espaciales)...")
    # Mantenemos los parámetros para garantizar estabilidad y balance de varianza
    ...
    joblib.dump(model, "modelos_entrenados/model2_random_forest.pkl")
    joblib.dump(scaler, "modelos_entrenados/model2_scaler.pkl")
